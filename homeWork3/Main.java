...
public class Main {
  public static void main(String[] args) {
    ...
    try {
      ...
      String path = "homeWork3\\" + people.get(0) + ".txt";
  ...
  public static ArrayList<String> turnStringToArrayWithCheckData(String str) throws RuntimeException {
    ...
    if (!phoneString.isEmpty()) {
      ...
      throw new RuntimeException("Не указзан номер телефона ");
    }
    if (!birthDate.isEmpty() & checkDate(birthDate)) {
      temp.add(birthDate);
    } else {
      throw new RuntimeException("Не указзана дата рождения ");
    }
    if (!gender.isEmpty()) {
      temp.add(gender);
    } else {
      throw new RuntimeException("Не указзана пол ");
    }
    return temp;
  }
  ...
  public static boolean checkDate(String str) {
    ...
    try {
      ...
      if (diffInYears < 0) {
        System.out.println("К сожалению вы еще не родились ");
        return false;
      }
      if (diffInYears / 365 > 100) {
        System.out.println("Приветсвтую долгожителей ");
      }
    } catch (Exception e) {
      System.out.println("Формат даты не соответствует ");
    }
    return true;
  }
}
